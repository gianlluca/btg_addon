...
def get_sprite_res(orthoscale, object, scene_ep):
	render_sizes = []
	...
	sprite_res = orthoscale * scene_ep.meter_pixel_scale * int(object.data.sprite_properties.super_sampling)

	if( int(object.data.sprite_properties.scale_type)):
		for res in render_sizes:
			if(res > sprite_res):
				sprite_res = res
				break
	else:
	...
	if sprite_res > scene_ep.max_render_size:
		sprite_res = scene_ep.max_render_size
...
def configure_render(object, center_location, sprite_path, scene_ep):
	...
	sprite_res = get_sprite_res(orthoscale, object, scene_ep)

	scene.render.resolution_x = sprite_res 
	scene.render.resolution_y = sprite_res

	scene.render.filter_size = object.data.sprite_properties.filter_size
	...
	sprite_scale = (orthoscale * scene_ep.meter_pixel_scale) / sprite_res 
...
def get_orthoscale_object(object, scene_ep):
	...
	return (bigger * 2.0) + (object.data.sprite_properties.margin_render  / scene_ep.meter_pixel_scale / int(object.data.sprite_properties.super_sampling)) * 2.0
...
def add_object_panel(object, col):
	...
	if(object.type == "EMPTY"):
		...
		if(object.data.godot_properties.mesh_type == 'Sprite'):
			row = col.row(align=True)
			row.prop(object.data.sprite_properties, 'margin_render',  text='Margin Render:',  expand=False)
			row = col.row(align=True)
			row.prop(object.data.sprite_properties, 'filter_size',     text='Filter Size:',     expand=False)
			row = col.row(align=True)
			row.prop(object.data.sprite_properties, 'super_sampling', text='Super Sampling:', expand=False)
			row = col.row(align=True)
			row.prop(object.data.sprite_properties, 'scale_type',     text='Scale Type:',     expand=False)
